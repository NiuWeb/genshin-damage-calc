...
import { FromWorker, WORKER_PATHS, ToChildWorker, MsgToChildWorker } from "./config"
...
export class OptimizerChild extends FrontendAction<FromWorker, ToChildWorker> {
    ...
    /** Sends a message to a child optimizer */
    SendMessage(data: MsgToChildWorker): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            const id1 = this.worker.Post(WORKER_PATHS.BACKEND_CHILD_RUN + "/msg", data as never)
            const listener = this.worker.AddListener(WORKER_PATHS.FRONTEND_CHILD_RUN + "/msg", (_, { id }) => {
                if (id !== id1) { return }
                resolve()
                this.worker.RemoveListener(listener)
            })
